...
@implementation SVGColor

- (instancetype)initWithR:(NSUInteger)red G:(NSUInteger)green B:(NSUInteger)blue
{
    self = [super init];
...
+ (instancetype)colorWithR:(NSUInteger)red G:(NSUInteger)green B:(NSUInteger)blue
{
    return [[SVGColor alloc] initWithR:red G:green B:blue];
}

+ (instancetype)colorWithRGBValue:(NSUInteger)rgbValue
{
    //    rgbaValue = rgbValue & 0xFFFFFF;
    
    NSUInteger red = (rgbValue & 0xFF0000) >> 16;
    NSUInteger green = (rgbValue & 0xFF00) >> 8;
    NSUInteger blue = rgbValue & 0xFF;
    
    return [[SVGColor alloc] initWithR:red G:green B:blue];
}

+ (SVGColor *)colorWithHexString:(NSString *)hexString
{
    ...
    SVGColor *color = [self colorWithR:redInt G:greenInt B:blueInt];
...
{
    SVGColor * color = [self colorsByName][text];
    
    if (color == nil)
    {
        color = [self colorWithHexString:text];
        
        if (color)
        {
            [self colorsByName][text] = color;
        }
    }
...
@synthesize UIColor = _UIColor;

- (UIColor *)UIColor
{
    if (_UIColor == nil)
    {
        _UIColor = [UIColor colorWithRed:(self.red / 255.f) green:(self.green / 255.f) blue:(self.blue / 255.f) alpha:1.f];
    }
    
    return _UIColor;
}

#define rgb(r,g,b) [SVGColor colorWithR:r G:g B:b]

+ (NSMutableDictionary *)colorsByName
{
